
<x-app-layout>
    <div class="row">
        <div class="col-12">
            <div class="card">
                <!-- card header -->
                <div class="card-header bg-primary">
                    <h4 class="card-title">Total Bank Report</h4>
                    <div>

                        <a href="{{ route('bank_info') }}" class="btn btn-sm btn-success"><i class="fa fa-reply"></i><span class="btn-icon-add"></span>Back</a>
                    </div>

                </div>
                <!-- card body -->
                <div class="card-body">
                    <table class="table table-bordered text-center">
                        <tr class="bg-light">
                            <th>Deposit</th>
                            <th>Withdraw</th>
                            <th>Balance</th>
                        </tr>
                        <tr>
                            <td><label for="" id="totalamount" class="font-weight-bold"></label></td>
                            <td><label for="" id="totalwidthrow" class="font-weight-bold"></label></td>
                            <td><label for="" id="totalbalance" class="font-weight-bold"></label></td>
                        </tr>
                    </table>
                    
                </div>
            </div>
        </div>
    </div>
</x-app-layout>




<script>
    var alldebit = "{{ $formattedAlldebit }}";
    var allcredit = "{{ $formattedAllcredit }}";
    var amount = parseFloat(alldebit.replace(/,/g, '')) - parseFloat(allcredit.replace(/,/g, ''));

    var totalDecimal = amount.toFixed(2).split('.')[1];
    if (parseInt(totalDecimal) >= 50) {
        amount = Math.ceil(amount);
    } else {
        amount = Math.floor(amount);
    }

    var formattedAmount = amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
    document.getElementById('totalamount').innerHTML = alldebit;
    document.getElementById('totalwidthrow').innerHTML = allcredit;
    document.getElementById('totalbalance').innerHTML = formattedAmount;
</script>





































