...
$(document).ready(function () {
    ...
    $(".edit-data").click(function () {
        ...
        $.ajax({
            url:'{{ route('get-designation-edit')}}',
            method:'GET',
            dataType:"JSON",
            data:{'id':itemId},
            success:function(response){
                console.log(response);
                $('#hiddenId').val(response.getDesignation.id);
                $('#DesigName').val(response.getDesignation.name);
                $('#DesigRemarks').val(response.getDesignation.description);
                $('#status').empty();
                $('#status').append('<option value="1" ' + (response.getDesignation.status == 1 ? 'selected' : '') + '>Active</option>');
                $('#status').append('<option value="2" ' + (response.getDesignation.status == 2 ? 'selected' : '') + '>InActive</option>');
    ...
    $(document).ready(function () {
        ...
        $(".view-data").click(function () {
            ...
            $.ajax({
                ...
                success:function(response){
                    console.log(response);
                    $('#DesigviewName').val(response.getDesignation.name);
                    $('#DesigviewRemarks').val(response.getDesignation.description);
                    $('#viewstatus').val(response.getDesignation.status == 1 ? 'Active' : 'Inactive');
                }
            });
