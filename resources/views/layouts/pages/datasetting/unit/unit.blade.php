        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">All Unit List</h4>
...
$(document).ready(function () {
    ...
    $(".edit-data").click(function () {
        ...
        $.ajax({
            url:'{{ route('get-unit-edit')}}',
            method:'GET',
            dataType:"JSON",
            data:{'id':itemId},
            success:function(response){
                console.log(response);
                $('#hiddenId').val(response.getunit.id);
                $('#unitName').val(response.getunit.name);
                $('#unitRemarks').val(response.getunit.description);
                $('#status').empty();
                $('#status').append('<option value="1" ' + (response.getunit.status == 1 ? 'selected' : '') + '>Active</option>');
                $('#status').append('<option value="2" ' + (response.getunit.status == 2 ? 'selected' : '') + '>InActive</option>');
        ...
        $(".view-data").click(function () {
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    $('#unitviewName').val(response.getunit.name);
                    $('#unitviewRemarks').val(response.getunit.description);
                    $('#viewstatus').val(response.getunit.status == 1 ? 'Active' : 'Inactive');
                }
            });
