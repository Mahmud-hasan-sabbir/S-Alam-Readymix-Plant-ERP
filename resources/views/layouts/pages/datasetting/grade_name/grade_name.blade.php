        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-primary">
                    <h4 class="card-title">All Grade Name List</h4>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h5 class="modal-title">Create Grade Name</h5>

                    <button type="button" class="close text-white" data-dismiss="modal"><span>&times;</span></button>
                </div>

                <form class="form-valide" action="{{ route('grade_store_name') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body py-2 px-4">
                        <div class="row mt-2">
                             <label class="col-md-3 col-form-label">Grade Name :
    ...
    <div class="modal fade bd-example" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form class="form-valide" action="{{ route('update_grade') }}" method="POST" enctype="multipart/form-data">
                    @csrf
    ...
    $(".edit-data").click(function () {
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                $('#hiddenId').val(response.getgrade.id);
                $('#unitName').val(response.getgrade.name);
                $('#unitRemarks').val(response.getgrade.description);
                $('#status').empty();
                $('#status').append('<option value="1" ' + (response.getgrade.status == 1 ? 'selected' : '') + '>Active</option>');
                $('#status').append('<option value="2" ' + (response.getgrade.status == 2 ? 'selected' : '') + '>InActive</option>');
        ...
        $(".view-data").click(function () {
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    $('#unitviewName').val(response.getgrade.name);
                    $('#unitviewRemarks').val(response.getgrade.description);
                    $('#viewstatus').val(response.getgrade.status == 1 ? 'Active' : 'Inactive');
                }
            });
